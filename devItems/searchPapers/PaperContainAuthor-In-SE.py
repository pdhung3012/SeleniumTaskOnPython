...
fopInput=fopRoot+'logConferences_SE/'
fonSearchName='PaperOfAllamanis-In-SE'
# lstKeyWords=[('machine translation',1),('gnn',1),('graph neural network',1)]
...
lstTotalOutput=['Num\t"Conference"\tYear\t"Track"\t"Title"\t"Author"\t"Link"']
index=0
    ...
    try:
        ...
        for j in range(0,len(arrContent)):
            ...
            if len(arrTabItem)==5:
                ...
                hitWords=[]
                # print(arrTabItem[3])
                numberComma=len(arrTabItem[3].split(','))+len(arrTabItem[3].split(';'))
                if 'Allamanis' in arrTabItem[3]:
                    strCsvOutLine = '\t'.join(
                        [str(index), '"' + conf + '"', str(year), '"' + arrTabItem[1] + '"', '"' + arrTabItem[2] + '"',
...
df_new=df_new.sort_values(by=['Year'],ascending=(False))
...
f1.write('{}'.format(fopSearchResultFolder))
f1.close()
